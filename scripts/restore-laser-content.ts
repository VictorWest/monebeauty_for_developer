// Reverts the laserkarvanpoisto page back to the pre-redesign content
// captured by backup-laser-content.ts, in case the client asks to roll back.
//
// Restores the laser Technology row's images/focal point, every locale of
// its TechnologyContent, and every ServiceOption + ServiceOptionContent
// belonging to its related Service, to exactly what the snapshot recorded.
// It does not touch the redesigned page components — those are reverted
// separately via git (see content/backups/laser-original-page.json's sibling
// note in the redesign PR description for which commit to revert to).
//
...
const INPUT = join(process.cwd(), "content/backups/laser-original-page.json");
...
async function main() {
  ...
  const technology = await prisma.technology.findUnique({
    where: { slug: snapshot.technology.slug },
  });
  if (!technology) throw new Error("laser technology not found");

  console.log(
    apply ? "Restoring Technology.images/focal point…" : "[dry run] Would restore Technology.images/focal point",
  );
  if (apply) {
  ...
  for (const content of snapshot.technology.contents) {
    console.log(
      apply
        ? `Restoring TechnologyContent[${content.locale}]…`
        : `[dry run] Would restore TechnologyContent[${content.locale}]`,
    );
    if (apply) {
      await prisma.technologyContent.update({
        where: {
          technologyId_locale: { technologyId: technology.id, locale: content.locale },
        },
        data: {
          name: content.name,
  ...
  if (snapshot.relatedService) {
    const service = await prisma.service.findUnique({
      where: { slug: snapshot.relatedService.slug },
    });
    ...
    for (const option of snapshot.relatedService.options) {
      ...
      console.log(
        apply
          ? `Restoring ServiceOption[${option.key}]…`
          : `[dry run] Would restore ServiceOption[${option.key}]`,
      );
      if (apply) {
      ...
      for (const content of option.contents) {
        ...
        if (apply) {
          await prisma.serviceOptionContent.update({
            where: {
              optionId_locale: { optionId: existingOption.id, locale: content.locale },
            },
            data: {
              group: content.group,
              name: content.name,
