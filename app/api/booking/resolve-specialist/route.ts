...
import { resolveAnySpecialist } from "@/lib/booking";
 ...
 * final POST /api/booking still re-validates that specialist + slot).
 */
export async function POST(req: NextRequest) {
  ...
  if (!service || !option || !start || Number.isNaN(Date.parse(start)))
    return NextResponse.json({ error: "invalid_request" }, { status: 400 });
  const dateStr = clinicDateFromInstant(new Date(start));

  try {
    const resolved = await resolveAnySpecialist({
      dateStr,
      serviceKey: service,
      optionKey: option,
      locale,
